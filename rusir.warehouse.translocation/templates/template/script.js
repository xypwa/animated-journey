...
if (typeof BI.RusirTableController == 'undefined') {
    BI.SCSGCurrencyList = {};
    BI.iblockLookup = {};
    BI.RusirTableController = function (tableElement, ajaxId, entityTypeId, entityId, columns, rows, options) {
    ...
    BI.RusirTableController.prototype = {
        ...
        onSaveBtn: function () {
            ...
            if('beforeCommit' in self.callbacks)
                self.callbacks.beforeCommit(response.data.data);
            // console.log(type)
            // debugger;
            BX.ajax.runComponentAction('b-integration:rusir.warehouse.translocation',
                'saveBeforeDistribute', { // Вызывается без постфикса Action
                    ...
                    data: {
                        ...
                        src: 'ACCEPT',
                        destination: type[2],
                        assigned: $(`select[name='${type[2]}_RESPONSIBLE']`).val()
